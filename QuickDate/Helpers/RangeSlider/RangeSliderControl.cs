...
namespace QuickDate.Helpers.RangeSlider
{
    ...
    public class RangeSliderControl : ImageView
    {
        ...
        public RangeSliderControl(Context context, IAttributeSet attrs, int defStyleAttr) : base(context, attrs, defStyleAttr)
        {
            Init(context, attrs);
        }

        public RangeSliderControl(Context context, IAttributeSet attrs, int defStyleAttr, int defStyleRes) : base(context, attrs, defStyleAttr, defStyleRes)
        {
            Init(context, attrs);
        }
        ...
        private void TrackTouchEvent(MotionEvent ev, bool step)
        {
            try
            {
                ...
                switch (_pressedThumb)
                {
                    case Thumb.Lower when !MinThumbHidden:
                        SetNormalizedMinValue(ScreenToNormalized(x), step, true);
                        break;
                    case Thumb.Upper when !MaxThumbHidden:
        ...
        private Thumb? EvalPressedThumb(float touchX)
        {
            try
            {
                ...
                switch (minThumbPressed)
                {
                    // if both thumbs are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the thumbs in a corner, not being able to drag them apart anymore.
                    case true when maxThumbPressed:
